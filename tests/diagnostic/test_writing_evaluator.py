...
def test_prompt_version_is_v3():
    ...
    assert "A clear overview is required for Band 6+" in SYSTEM_PROMPT
...
def test_groq_reconciles_overall_band_when_drift_exceeds_threshold():
    ...
    async def run():
        with (
            patch("app.diagnostic.writing_evaluator.get_settings") as settings,
            patch(
                "app.diagnostic.writing_evaluator.chat_completion_json",
                new_callable=AsyncMock,
                return_value=(json.dumps(mismatched), {"choices": []}),
            ),
        ):
            settings.return_value.groq_model = "llama-3.3-70b-versatile"
            return await _call_groq_evaluation(
                task_part=1,
                question="Chart question",
                essay=VALID_ESSAY,
            )

    evaluation, raw_store, prompt_version, _model = asyncio.run(run())

    assert prompt_version == "v3"
    assert evaluation.overall_band == 6.0
    assert isinstance(raw_store, dict)
    assert raw_store["overall_band_reconciled"] is True
    assert raw_store["ai_overall_band"] == 7.5
    assert raw_store["calculated_overall_band"] == 6.0
...
def test_essay_hash_cache_hit_skips_groq():
    ...
    with (
        ...
        patch("app.diagnostic.writing_evaluator.chat_completion_json", new_callable=AsyncMock) as groq,
        patch("app.diagnostic.writing_evaluator.record_evaluate_writing_rate_limit") as rate,
    ):
...
def test_groq_failure_returns_503():
    request = MagicMock()
    body = MagicMock(
    ...
    with (
        ...
        patch("app.diagnostic.writing_evaluator.get_settings") as settings,
        patch(
            "app.diagnostic.writing_evaluator._call_groq_evaluation",
            new_callable=AsyncMock,
            side_effect=RuntimeError("Groq evaluation failed"),
        ),
    ):
...
def test_groq_retry_on_invalid_json_then_success():
    ...
    async def groq_side_effect(**_kwargs):
        calls.append(1)
        if len(calls) == 1:
            return "not-json", {"choices": []}
        return json.dumps(VALID_EVAL_JSON), {"choices": []}
...
def test_evaluate_writing_api_response_schema():
    ...
    with patch(
        "app.routers.diagnostic.evaluate_diagnostic_writing",
        new_callable=AsyncMock,
...
def test_submit_review_missing_ai_evaluation_returns_400():
    from app.diagnostic.schemas import DiagnosticReviewSubmitRequest
    from app.diagnostic.submit_review import submit_diagnostic_review
    ...
    with patch("app.diagnostic.submit_review._lookup_writing_evaluation", return_value=None):
        with pytest.raises(HTTPException) as exc:
            asyncio.run(submit_diagnostic_review(body))
    assert exc.value.status_code == 400
    assert "Writing AI evaluation required" in str(exc.value.detail)
